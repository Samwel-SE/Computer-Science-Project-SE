...
def threaded_client(conn, client_num, maps):
    ...
    try: 
        conn.send(str.encode(stringify_round_start_data(client_num, starting_player_data[client_num], maps[map_counter])))
        reply = ""
        ...
        try:
            ...
            # checks using the state checker variable recieved from client is in state 2 IE a player has been killed
                if data[-1] == 2:
                    ...
                    data_to_be_sent = stringify_round_start_data(client_num, starting_player_data[client_num], maps[map_counter])
